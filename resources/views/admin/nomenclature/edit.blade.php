...
<div class="nova-card-table" style="margin: 0 auto; padding: 30px; max-width: 800px;">
    ...
    <form action="{{ route('admin.nomenclatures.update', $nomenclature->id) }}" method="POST">
        ...
        @if($nomenclature->type === 'item')
            <div style="margin-bottom: 15px;">
                ...
                <select name="category_type" id="categoryTypeSelect" class="nova-input" style="width: 100%; padding: 8px 12px; border: 1px solid #cbd5e1; border-radius: 4px;" onchange="toggleFields()">
                    <option value="Материалы" {{ old('category_type', $nomenclature->category_type) === 'Материалы' ? 'selected' : '' }}>Материалы</option>
                    <option value="Товары" {{ old('category_type', $nomenclature->category_type) === 'Товары' ? 'selected' : '' }}>Товары</option>
                    <option value="Услуги" {{ old('category_type', $nomenclature->category_type) === 'Услуги' ? 'selected' : '' }}>Услуги</option>
                </select>
            </div>
        @else
        ...
        <div style="margin-bottom: 15px;">
            ...
            <input type="text" name="name" class="nova-input" style="width: 100%; padding: 8px 12px; border: 1px solid #cbd5e1; border-radius: 4px;" required value="{{ old('name', $nomenclature->name) }}">
        </div>
        ...
        @if($nomenclature->type === 'item')
            ...
            <div id="stockFieldsGroup">
                ...
                <div style="margin-bottom: 15px;">
                    ...
                    <input type="number" step="0.01" name="purchase_price" class="nova-input" style="width: 100%; padding: 8px 12px; border: 1px solid #cbd5e1; border-radius: 4px;" value="{{ old('purchase_price', $nomenclature->prices->sortByDesc('effective_date')->first()?->purchase_price ?? 0) }}">
                </div>
            </div>

            <div style="margin-bottom: 15px;">
                <label style="display: block; font-size: 14px; font-weight: 500; color: #0f172a; margin-bottom: 5px;">Цена продажи</label>
                <input type="number" step="0.01" name="selling_price" class="nova-input" style="width: 100%; padding: 8px 12px; border: 1px solid #cbd5e1; border-radius: 4px;" value="{{ old('selling_price', $nomenclature->prices->sortByDesc('effective_date')->first()?->selling_price ?? 0) }}">
            </div>
