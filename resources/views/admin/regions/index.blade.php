...
<div class="nova-card-table" style="margin: 0 auto; padding: 30px;">
    ...
    <button onclick="window.location.href='{{ route('admin.regions.create') }}'" class="nova-input" style="background-color: #3b82f6; color: #ffffff; cursor: pointer; margin-bottom: 10px; padding: 5px 20px; border: none; border-radius: 4px;">Добавить регион</button>
    
    <div class="table-header">Список регионов</div>
            ...
            <tbody>
                @foreach($regions as $region)
                    <tr style="border-bottom: 1px solid #f1f5f9; font-size: 14px; color: #0f172a;">
                        <td style="padding: 10px;">{{ $region->id }}</td>
                        <td style="padding: 10px; font-weight: 500;">{{ $region->title }}</td>
                        <td style="padding: 10px; color: #64748b;">{{ $region->slug }}</td>
                        <td style="padding: 10px; text-align: right;">
                            <a href="{{ route('admin.regions.edit', $region->id) }}" style="color: #3b82f6; text-decoration: none; margin-right: 10px;">Изменить</a>
                            <button type="button" onclick="openDeleteModal('{{ route('admin.regions.destroy', $region->id) }}', '{{ $region->title }}')" style="background: none; border: none; color: #ef4444; cursor: pointer; padding: 0;">Удалить</button>
                        </td>
                    </tr>
                @endforeach
            </tbody>
...
<script>
    function openDeleteModal(actionUrl, regionTitle) {
        const modal = document.getElementById('deleteModal');
        const form = document.getElementById('deleteForm');
        const text = document.getElementById('deleteModalText');
        
        form.action = actionUrl;
        text.innerText = `Вы действительно хотите удалить регион «${regionTitle}»? Это действие нельзя отменить.`;
