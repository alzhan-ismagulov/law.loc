...
<div class="nova-card-table" style="padding: 25px;">
    <div class="table-header" style="margin-bottom: 20px;">Добавление клиента</div>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 15px; font-size: 14px;">
                <div>
                    ...
                    <select name="type" class="nova-input" style="width: 100%; padding: 8px;">
                        <option value="individual" {{ old('type') == 'individual' ? 'selected' : '' }}>Физлицо</option>
                        <option value="company" {{ old('type') == 'company' ? 'selected' : '' }}>Компания</option>
                    </select>
                </div>
                <div>
                    <label style="display: block; margin-bottom: 5px; color: #64748b;">Имя / Название компании *</label>
                    <input type="text" name="name" value="{{ old('name') }}" class="nova-input" style="width: 100%; padding: 8px;" required>
                </div>
                <div>
                    <label style="display: block; margin-bottom: 5px; color: #64748b;">ИИН / БИН (12 знаков)</label>
                    <input type="text" name="bin_iin" id="binIinInput" maxlength="12" value="{{ old('bin_iin') }}" class="nova-input" style="width: 100%; padding: 8px;" placeholder="12 цифр">
                </div>
            </div>
        </div>
        ...
        <div style="background: #f8fafc; border: 1px solid #e2e8f0; padding: 20px; border-radius: 8px; margin-bottom: 20px;">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 15px; font-size: 14px;">
                <div>
                    ...
                    <input type="email" name="email" value="{{ old('email') }}" class="nova-input" style="width: 100%; padding: 8px;" required>
                </div>
                <div>
                    <label style="display: block; margin-bottom: 5px; color: #64748b;">Телефон *</label>
                    <input type="text" name="phone" id="phoneInput" value="{{ old('phone') }}" class="nova-input" style="width: 100%; padding: 8px;" placeholder="+7 (777) 123-45-67" required>
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="text" name="city" value="{{ old('city') }}" class="nova-input" style="width: 100%; padding: 8px;" required>
                </div>
                <div>
                    <label style="display: block; margin-bottom: 5px; color: #64748b;">Адрес</label>
                    <input type="text" name="address" value="{{ old('address') }}" class="nova-input" style="width: 100%; padding: 8px;">
                </div>
                <div>
                    <label style="display: block; margin-bottom: 5px; color: #64748b;">Контактное лицо (для юрлиц)</label>
                    <input type="text" name="contact_person" value="{{ old('contact_person') }}" class="nova-input" style="width: 100%; padding: 8px;">
                </div>
                <div>
                    <label style="display: block; margin-bottom: 5px; color: #64748b;">Должность</label>
                    <input type="text" name="position" value="{{ old('position') }}" class="nova-input" style="width: 100%; padding: 8px;">
                </div>
            </div>
        </div>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr 1fr 1fr; gap: 15px; font-size: 14px;">
                <div>
                    ...
                    <select name="status" class="nova-input" style="width: 100%; padding: 8px;">
                        ...
                        <option value="lead" {{ old('status', 'lead') == 'lead' ? 'selected' : '' }}>Лид</option>
                        <option value="archive" {{ old('status', 'archive') == 'archive' ? 'selected' : '' }}>Архив</option>
                    </select>
                </div>
                <div>
                    <label style="display: block; margin-bottom: 5px; color: #64748b;">Источник привлечения</label>
                    <input type="text" name="source" value="{{ old('source') }}" class="nova-input" style="width: 100%; padding: 8px;">
                </div>
                <div>
                    <label style="display: block; margin-bottom: 5px; color: #64748b;">Персональная скидка (%)</label>
                    <input type="number" step="0.01" name="discount_percent" value="{{ old('discount_percent', 0) }}" class="nova-input" style="width: 100%; padding: 8px;">
                </div>
                <div>
                    <label style="display: block; margin-bottom: 5px; color: #64748b;">Банк</label>
                    <input type="text" name="bank_name" value="{{ old('bank_name') }}" class="nova-input" style="width: 100%; padding: 8px;">
                </div>
                <div style="grid-column: span 2;">
                    <label style="display: block; margin-bottom: 5px; color: #64748b;">IBAN (20 символов)</label>
                    <input type="text" name="iban" id="ibanInput" maxlength="20" value="{{ old('iban') }}" class="nova-input" style="width: 100%; padding: 8px;" placeholder="KZ... (только латиница и цифры)">
                </div>
            </div>
        </div>
        ...
        <div style="background: #f8fafc; border: 1px solid #e2e8f0; padding: 20px; border-radius: 8px; margin-bottom: 20px;">
            ...
            <div style="display: grid; grid-template-columns: 1fr; gap: 15px; font-size: 14px;">
                <div style="max-width: 400px;">
                    <label style="display: block; margin-bottom: 5px; color: #64748b;">Пароль для входа *</label>
                    <input type="password" name="password" class="nova-input" style="width: 100%; padding: 8px;" required>
                </div>
                <div>
                    <label style="display: block; margin-bottom: 5px; color: #64748b;">Внутренние заметки</label>
                    <textarea name="internal_notes" class="nova-input" style="width: 100%; padding: 8px; height: 120px;">{{ old('internal_notes') }}</textarea>
                </div>
            </div>
        </div>

        <button type="submit" style="background: #3b82f6; color: white; padding: 10px 20px; border: none; border-radius: 4px; cursor: pointer; font-size: 14px;">Создать клиента</button>
    </form>
</div>
