...
<div class="nova-card-table" style="margin: 0 auto; padding: 30px; max-width: 700px;">
    ...
    <form action="{{ route('admin.translations.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div style="margin-bottom: 15px;">
            ...
            <input type="date" name="order_date" class="nova-input" style="width: 100%; padding: 8px 12px; border: 1px solid #cbd5e1; border-radius: 4px;" value="{{ date('Y-m-d') }}" required>
        </div>
        ...
        <div style="margin-bottom: 15px;">
            ...
            <select name="service_type" id="service_type_select" class="nova-input" style="width: 100%; padding: 8px 12px; border: 1px solid #cbd5e1; border-radius: 4px;" required>
                <option value="written">Письменный (за 1800 знаков)</option>
                <option value="oral">Устный (за час)</option>
                <option value="sync">Синхронный (за час)</option>
                <option value="notary">Нотариальное заверение</option>
        ...
        <div style="margin-bottom: 15px;">
            ...
            <select name="translator_id" id="translator_select" class="nova-input" style="width: 100%; padding: 8px 12px; border: 1px solid #cbd5e1; border-radius: 4px;" required>
                <option value="">-- Сначала выберите переводчика --</option>
        ...
        <div style="margin-bottom: 15px;">
            ...
            <select name="nomenclature_id" id="nomenclature_select" class="nova-input" style="width: 100%; padding: 8px 12px; border: 1px solid #cbd5e1; border-radius: 4px;" required>
                ...
                @foreach($nomenclatureServices ?? [] as $service)
                    <option value="{{ $service->id }}" data-selling-price="{{ $service->currentPrice?->selling_price ?? 0 }}">
        ...
        <div id="file_upload_block" style="margin-bottom: 20px;">
            <label style="display: block; font-size: 14px; font-weight: 500; color: #0f172a; margin-bottom: 5px;">Файлы оригинала (можно выбрать несколько)</label>
        ...
        <div id="chars_block" style="margin-bottom: 15px;">
            ...
            <input type="number" id="chars_count_input" name="chars_count" class="nova-input" style="width: 100%; padding: 8px 12px; border: 1px solid #cbd5e1; border-radius: 4px;" value="0" min="0">
        </div>
        ...
        <div style="margin-bottom: 15px;">
            ...
            <textarea name="notes" rows="3" class="nova-input" style="width: 100%; padding: 8px 12px; border: 1px solid #cbd5e1; border-radius: 4px; font-weight: normal; font-style: normal; text-transform: none;" placeholder="Введите примечание словами..."></textarea>
        </div>
        ...
        <div style="margin-bottom: 15px;">
            ...
            <input type="number" step="0.01" id="client_price_input" name="client_price" class="nova-input" style="width: 100%; padding: 8px 12px; border: 1px solid #cbd5e1; border-radius: 4px;" value="0" required>
        </div>
        ...
        <div style="margin-bottom: 15px;">
            ...
            <input type="number" step="0.01" id="translator_price_input" name="translator_price" class="nova-input" style="width: 100%; padding: 8px 12px; border: 1px solid #cbd5e1; border-radius: 4px;" value="0" required>
        </div>

        <div style="display: flex; gap: 10px;">
            <button type="submit" class="nova-input" style="background-color: #3b82f6; color: #ffffff; cursor: pointer; padding: 8px 20px; border: none; border-radius: 4px; width: auto;">Создать заказ</button>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Автоматический подсчет знаков для ВСЕХ выбранных файлов
        if (fileInput) {
                    ...
                    for (let i = 0; i < this.files.length; i++) {
                        ...
                        try {
                            ...
                            if (data.chars_count !== undefined) {
                                totalChars += parseInt(data.chars_count) || 0;
                            }
                        } catch (e) {
