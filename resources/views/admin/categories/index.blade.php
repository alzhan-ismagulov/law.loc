...
<div class="nova-card-table" style="margin: 0 auto; padding: 30px;">
    ...
    <button onclick="window.location.href='{{ route('admin.categories.create') }}'" class="nova-input" style="background-color: #3b82f6; color: #ffffff; cursor: pointer; margin-bottom: 10px; padding: 5px 20px; border: none; border-radius: 4px;">Добавить категорию</button>
        ...
        <tbody>
            @foreach($categories as $category)
                <tr style="border-bottom: 1px solid #f1f5f9; font-size: 14px; color: #0f172a;">
                    ...
                    <td style="padding: 10px; font-weight: 500;">{{ $category->title }}</td>
                    <td style="padding: 10px; color: #64748b;">{{ $category->slug }}</td>
                    <td style="padding: 10px; color: #64748b;">{{ $category->description }}</td>
                    <td style="padding: 10px; text-align: right;">
                        <a href="{{ route('admin.categories.edit', $category->id) }}" style="color: #3b82f6; text-decoration: none; margin-right: 10px;">Изменить</a>
                        <button type="button" onclick="openDeleteModal('{{ route('admin.categories.destroy', $category->id) }}', '{{ $category->title }}')" style="background: none; border: none; color: #ef4444; cursor: pointer; padding: 0;">Удалить</button>
                    </td>
                </tr>
            @endforeach
        </tbody>
...
<script>
    function openDeleteModal(actionUrl, categoryName) {
        ...
        text.innerText = `Вы действительно хотите удалить категорию «${categoryName}»? Это действие нельзя отменить.`;
