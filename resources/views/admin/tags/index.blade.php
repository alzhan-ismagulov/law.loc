...
<div class="nova-card-table" style="margin: 0 auto; padding: 30px;">
    ...
    <button onclick="window.location.href='{{ route('admin.tags.create') }}'" class="nova-input" style="background-color: #3b82f6; color: #ffffff; cursor: pointer; margin-bottom: 10px; padding: 5px 20px; border: none; border-radius: 4px;">Добавить тег</button>
    
    <div class="table-header"">Список тегов</div>
        ...
        <tbody>
            @foreach($tags as $tag)
                <tr style="border-bottom: 1px solid #f1f5f9; font-size: 14px; color: #0f172a;">
                    ...
                    <td style="padding: 10px; font-weight: 500;">{{ $tag->title }}</td>
                    <td style="padding: 10px; color: #64748b;">{{ $tag->slug }}</td>
                    <td style="padding: 10px; text-align: right;">
                        <a href="{{ route('admin.tags.edit', $tag->id) }}" style="color: #3b82f6; text-decoration: none; margin-right: 10px;">Изменить</a>
                        <button type="button" onclick="openDeleteModal('{{ route('admin.tags.destroy', $tag->id) }}', '{{ $tag->title }}')" style="background: none; border: none; color: #ef4444; cursor: pointer; padding: 0;">Удалить</button>
                    </td>
                </tr>
            @endforeach
        </tbody>
